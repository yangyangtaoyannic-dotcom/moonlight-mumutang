...
//    private InputCaptureProvider inputCaptureProvider;
    ...
    private SBSStreamView streamView;
    ...
    private ServiceConnection usbDriverServiceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
            ...
            binder.setStateListener(GameSbs.this);
            binder.start();
...
//        inputCaptureProvider = InputCaptureManager.getInputCaptureProvider(this, this);
        ...
        appName = GameSbs.this.getIntent().getStringExtra(EXTRA_APP_NAME);
        pcName = GameSbs.this.getIntent().getStringExtra(EXTRA_PC_NAME);

        String host = GameSbs.this.getIntent().getStringExtra(EXTRA_HOST);
        int port = GameSbs.this.getIntent().getIntExtra(EXTRA_PORT, NvHTTP.DEFAULT_HTTP_PORT);
        int httpsPort = GameSbs.this.getIntent().getIntExtra(EXTRA_HTTPS_PORT, 0); // 0 is treated as unknown
        int appId = GameSbs.this.getIntent().getIntExtra(EXTRA_APP_ID, StreamConfiguration.INVALID_APP_ID);
        String uniqueId = GameSbs.this.getIntent().getStringExtra(EXTRA_UNIQUEID);
        boolean appSupportsHdr = GameSbs.this.getIntent().getBooleanExtra(EXTRA_APP_HDR, false);
        byte[] derCertData = GameSbs.this.getIntent().getByteArrayExtra(EXTRA_SERVER_CERT);
        ...
        NvApp app=new NvApp(appName != null ? appName : "app", appId, appSupportsHdr);
...
//                }, 500);
                ...
                UiHelper.notifyStreamConnected(GameSbs.this);
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
        ...
        if (!connected && !connecting) {
            ...
            renderer = new VideoTextureRenderer(this, streamView.getSurfaceTexture(), surfaceWidth, surfaceHeight, this);
            renderer.setVideoSize(prefConfig.width, prefConfig.height);

            LimeLog.info("----surface:" + surfaceWidth + " x " + surfaceHeight + ";prefConfig=" + prefConfig.width + " x " + prefConfig.height);
